export function PaginationDirective($location) {
  'ngInject';
  ...
  let directive = {
    ...
    link: function (scope, element) {
      ...
      function isDisabled(page) {
        return page.type === 'separator' || page.type === 'prev' && current === 1 || page.type === 'next' && current === pagesCount;
      }
      ...
      function toPage(page) {
        ...
        if (page.type === 'page') {
          ...
          if (current > 1) current--;else return;
        } else if (page.type === 'next') {
          if (current < pagesCount) current++;else return;
        }
      ...
      function buildOptions() {
        ...
        if (scope.maxSize) {
          if (scope.maxSize < 7) options.maxSize = 7;else options.maxSize = scope.maxSize;
        }
      ...
      function generate(pages, options) {
        ...
        if (pages.count <= options.maxSize) list = generateFlat(pages);else list = generateFull(pages);
  ...
  function template() {
    return '<div class="pagination">' + '<div class="page"' + 'ng-repeat="page in pages"' + 'ng-class="{ \'active\': isCurrent(page), \'disabled\': isDisabled(page) }"' + 'ng-click="toPage(page)">' + '{{ page.value }}' + '</div>' + '</div>';
  }
}

//# sourceMappingURL=pagination.directive-compiled.js.map